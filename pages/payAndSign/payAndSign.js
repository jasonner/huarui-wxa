...
Page({
    ...
    hrp.getPayInfo({ "applicationNo": this.data.params.applicationNo}).then((data) => {
      ...
      if(data.result == "0"){
        ...
        if (data.payProduct.payType.length == 1 && data.payProduct.payType[0] == 'wxPay') {
          if (data.payOrder.exchangePoint != "" && data.payOrder.exchangePoint){
            this.setData({
              exchangePointShow:true,
              exchangePoint:data.payOrder.exchangePoint
            })
          }
        this.setData({
          payTypeName: '微信支付',
  ...
  wxConfirmBuy:function(){
    ...
    hrp.payAndSign({
      "applicationNo": that.data.params.applicationNo, "payMode": "wxaPay", accountNo: third,
      "system": "","type": "",
