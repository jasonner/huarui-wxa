...
Page({
  ...
  calPoint: function () {
    ...
    if (orderAmount == '-1') {
      ...
      if (parseInt(orderAmount) <parseInt( minPrice)) {
        this.setData({
          pointChecked: false,
          switchOpen: true,
  ...
  getAge: function (birthDate) {
    ...
    if (birthDate == undefined) {
      ...
      var insureInfovalue = wx.getStorageSync('insureInfoDetail');
      var insuredAgeType = insureInfovalue.insuredAgeType; // 指定日期生效
  ...
  insureBTN: function (options) {
   ...
   console.log(insureInfoDetail.insuredType);
    ...
    } else if (!this.checkInsurdBirth(buyInfo.birthday)) {
      this.errTip("被保人年龄不在被保范围内");
